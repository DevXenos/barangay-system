...
<body>
	...
	<!-- PRINT BUTTON -->
	<button id="printBtn">🖨️ Print Certificate of Residency</button>
	...
	<div id="print-area" style="width: 8.5in; height: 11in; padding: 60px; border: 2px solid #000; margin: auto; position: relative; box-sizing: border-box; background-color: #fff;">
		...
		<div style="text-align: center; line-height: 1.3; position: relative;">
			...
			<h2 style="text-decoration: underline; font-weight: bold; margin: 10px 0;">CERTIFICATE OF RESIDENCY</h2>
		</div>
		...
		<div style="margin-top: 40px; text-align: justify; font-size: 16px; line-height: 1.8;">
			...
			<?php
			// Format residency start date nicely
			$formatted_start_date = 'Unknown';
			if (!empty($residency_start_date) && $residency_start_date !== '0000-00-00') {
				$timestamp = strtotime($residency_start_date);
				$formatted_start_date = date('F j, Y', $timestamp);
			}
			?>

			<p style="text-indent: 60px;">
				This is to certify that <span style="font-weight: bold; text-transform: uppercase;"><?= $full_name ?></span>,
				of legal age, <?= $gender ?>, currently residing at <span style="font-weight: bold;"><?= $address ?></span>,
				is a bona fide resident of Barangay <span style="font-weight: bold;"><?= $barangayName ?></span>, Municipality of <span style="font-weight: bold;"><?= $municipality ?></span>, Province of <span style="font-weight: bold;"><?= $province ?></span>.
			</p>

			<p style="text-indent: 60px;">
				This certification is issued upon the request of the above-named person for whatever legal purpose it may serve, and confirms that he/she has been residing in this barangay since <span style="font-weight: bold;"><?= $formatted_start_date ?></span>.
			</p>
		...
		<div style="margin-top: 80px;">
			<div style="display: flex; justify-content: space-between; padding: 0 80px;">
				<div style="text-align: center;">
					...
					<p style="margin: 0; font-size: 14px;">Resident / Applicant</p>
				</div>
