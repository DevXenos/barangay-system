...
const setTableAction = () => {
	...
	$("#staffTable").find(".btn-edit").off("click").on("click", (e) => {
		...
		showDialog<StaffEditType>("addNewStaffModal", {
			onShow({ form }) {
			...
			onSubmit({ data, modal }) {
				const payload: StaffEditType = { ...data, id };

				// Only send password if filled
				if (!payload.password) delete payload.password;

				// Use setData to mark action = put
...
$("#newNewStaffBtn").on("click", () => {
	showDialog<StaffType>("addNewStaffModal", {
		onShow({ form }) {
