-- Drop and recreate database
DROP DATABASE IF EXISTS `barangay`;

CREATE DATABASE IF NOT EXISTS `barangay`;

USE `barangay`;

-- Residents table
CREATE TABLE IF NOT EXISTS `residents` (
    `id` VARCHAR(50) PRIMARY KEY, -- corresponds to EntityType<'resident'>
    `first_name` VARCHAR(100) NOT NULL,
    `middle_name` VARCHAR(100) DEFAULT NULL,
    `last_name` VARCHAR(100) NOT NULL,
    `email` VARCHAR(150) NOT NULL UNIQUE,
    `password` VARCHAR(150) NOT NULL,
    `birth_date` DATE NOT NULL,
    `contact_number` VARCHAR(20) NOT NULL,
    `gender` ENUM('Male', 'Female', 'Other') NOT NULL,
    `civil_status` ENUM(
        'Single',
        'Married',
        'Widowed',
        'Separated',
        'Divorced'
    ) NOT NULL,
    `address` VARCHAR(255) NOT NULL,
    `token` VARCHAR(255) NOT NULL,
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);

-- Staffs table
CREATE TABLE IF NOT EXISTS `staffs` (
    `id` VARCHAR(50) PRIMARY KEY, -- corresponds to EntityType<'staff'>
    `first_name` VARCHAR(100) NOT NULL,
    `last_name` VARCHAR(100) NOT NULL,
    `email` VARCHAR(150) NOT NULL UNIQUE,
    `password` VARCHAR(150) NOT NULL,
    `phone_number` VARCHAR(20) NOT NULL,
    `role` ENUM('Staff', 'Administrator') NOT NULL,
    `token` VARCHAR(255) DEFAULT NULL,
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);

-- Announcements table
CREATE TABLE IF NOT EXISTS `announcements` (
    `id` VARCHAR(50) PRIMARY KEY, -- corresponds to EntityType<'announcement'>
    `title` VARCHAR(255) NOT NULL,
    `description` TEXT NOT NULL,
    `date` DATE NOT NULL,
    `time` TIME DEFAULT NULL,
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP
);

-- Requests table
CREATE TABLE IF NOT EXISTS `requests` (
    `id` VARCHAR(50) PRIMARY KEY NOT NULL,
    /* Request Number request_ */ 
    `request_for` ENUM(
        'Self',
        'Child',
        'Husband/Wife'
    ) NOT NULL,
    `resident_id` VARCHAR(50) NOT NULL,
    `document_type` VARCHAR(50) NOT NULL,
    `first_name` VARCHAR(50) NOT NULL,
    /* FROM SELF, OR CHILD, OR HUSBAND/WIFE */ `middle_name` VARCHAR(50) NOT NULL,
    `last_name` VARCHAR(50) NOT NULL,
    `birth_date` VARCHAR(50) NOT NULL,
    `gender` ENUM('Male', 'Female', 'Other') NOT NULL,
    `civil_status` ENUM(
        'Single',
        'Married',
        'Widowed',
        'Separated',
        'Divorced'
    ) NOT NULL,
    `address` VARCHAR(50) NOT NULL,
    `contact_number` VARCHAR(50) NOT NULL,
    `purpose` TEXT NOT NULL,
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT `fk_request_resident` FOREIGN KEY (`resident_id`) REFERENCES `residents` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Default administrator account
INSERT INTO
    `staffs` (
        id,
        first_name,
        last_name,
        email,
        password,
        phone_number,
        role
    )
VALUES (
        'staff_admin',
        'FirstName',
        'LastName',
        'admin@gmail.com',
        '$2y$10$2hPenp1HG2l4QmqmqGlsyOn6Vc0qw82AwFkVbXGCiw/DiVBMh9Wzm',
        '09123456789',
        'administrator'
    );

ALTER TABLE `requests`
ADD COLUMN `status` ENUM(
    'Pending',
    'Approved',
    'Rejected',
    'Cancelled'
) DEFAULT 'Pending' AFTER `purpose`;